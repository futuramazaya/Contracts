...
// MasterChef is the master of Trip. He can make Trip and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once Trip is sufficiently
...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //   1. The pool's `accTripPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardBlock;  // Last block number that Trips distribution occurs.
    ...
    // Trip tokens created per block.
    uint256 public TripPerBlock;
    // Bonus muliplier for early Trip makers.
    ...
    // The block number when Trip mining starts.
    ...
    // Update the given pool's Trip allocation point and deposit fee. Can only be called by the owner.
    ...
    function pendingTrip(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 TripReward = multiplier.mul(TripPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 TripReward = multiplier.mul(TripPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        Trip.mint(devaddr, TripReward.mul(909).div(10000));//Dev gets 9.09%
        Trip.mint(address(this), TripReward);
    ...
    function safeTripTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > TripBal) {
            transferSuccess = Trip.transfer(_to, TripBal);
        } else {
            transferSuccess = Trip.transfer(_to, _amount);
        }
        require(transferSuccess, "safeTripTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _TripPerBlock) public onlyOwner {
        massUpdatePools();
        TripPerBlock = _TripPerBlock;
        emit UpdateEmissionRate(msg.sender, _TripPerBlock);
    }
    //For limiting totalSupply of token
    function capChange(uint256 _amount) public onlyOwner {
        Trip.changeCap(_amount);
    }
}
