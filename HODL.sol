...
contract HODL is Context, IERC20, Ownable {
    ...
    //Initial fee of  32%, split as 40%,40%,20% . Additional zero for decimal correction  
    //Will be reduced 4,4,2 every day until 22 days for each individual user (1% per day)
    uint256 public _taxFee = 1280;
...
//Messy code to avoid stack to deep :)
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        rTransferAmount = rTransferAmount.sub(calculateBurnFee(tAmount).mul(currentRate));
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        if(!takeFee){
            ...
            _taxFee = _taxFee.sub(40 * multiplier);
