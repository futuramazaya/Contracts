...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public dividendToken = 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82; //Cake token
...
contract CakeRise is ERC20, Ownable {
    using SafeMath for uint256;

    IPancakeswapV2Router02 public pancakeswapV2Router;
    address public immutable pancakeswapV2Pair;

    address public _dividendToken = 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82; //Cake token
    ...
    CakeRiseDividendTracker public dividendTracker;
    ...
    uint256 public maxWalletToken = 1500000000 * (10**18); 
    ...
    constructor(uint256 _dividendRewardsFee, uint256 _marketingFee, address _buyBackWallet) 
        ERC20("CakeRise", "CAKERISE") 
    {
        ...
        liquidityFee = 2;
        burnFee = 1;
        totalFees = _dividendRewardsFee.add(_marketingFee).add(liquidityFee).add(burnFee);

    	dividendTracker = new CakeRiseDividendTracker();

    	buyBackWallet = _buyBackWallet;
    	
    	IPancakeswapV2Router02 _pancakeswapV2Router = IPancakeswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E); //0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3
    ...
    receive() external payable {
  	...
  	function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
  	    require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 200, 
  	    "CakeRise: Sell transaction multipler must be between 100 (1x) and 200 (2x)");
  	    sellFeeIncreaseFactor = multiplier;
  	}
  	
  	function setMarketingDivisor(uint256 divisor) external onlyOwner {
  	    require(divisor >= 0 && divisor <= 100, "CakeRise: Marketing divisor must be between 0 (0%) and 100 (100%)");
  	...
  	function prepareForPreSale() external onlyOwner {
        ...
        liquidityFee = 0;
        burnFee = 0;
        totalFees = dividendRewardsFee.add(marketingFee).add(liquidityFee).add(burnFee);
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "CakeRise: A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "CakeRise: The dividend tracker already has that address");

        CakeRiseDividendTracker newDividendTracker = CakeRiseDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), 
        "CakeRise: The new dividend tracker must be owned by the CAKERISE token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "CakeRise: Dividend reward tax must be between 0 and 10");
        dividendRewardsFee = newFee;
        totalFees = dividendRewardsFee.add(marketingFee).add(liquidityFee).add(burnFee);
    }
    
    function updateMarketingFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "CakeRise: Marketing tax must be between 0 and 10");
        marketingFee = newFee;
        totalFees = dividendRewardsFee.add(marketingFee).add(liquidityFee).add(burnFee);
    }

    function updateBurnFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "CakeRise: Burn tax must be between 0 and 10");
        burnFee = newFee;
        totalFees = dividendRewardsFee.add(marketingFee).add(liquidityFee).add(burnFee);
    }

    function updateLiquiddityFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "CakeRise: Liquidity tax must be between 0 and 10");
        liquidityFee = newFee;
        totalFees = dividendRewardsFee.add(marketingFee).add(liquidityFee).add(burnFee);
    }

    function updatePancakeswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(pancakeswapV2Router), "CakeRise: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "CakeRise: Account is already excluded");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != pancakeswapV2Pair, "CakeRise: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "CakeRise: Automated market maker pair is already set to that value");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "CakeRise: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "CakeRise: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "CakeRise: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if (
            ...
            if (!swapping && canSwap) {
                ...
                uint256 buyBackBalance = address(this).balance;
                if (buyBackEnabled && buyBackBalance >= uint256(1 ether)) {
...
contract CakeRiseDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("CakeRise_Dividend_Tracker", "CakeRise_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "CakeRise_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "CakeRise_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main CakeRise contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "CakeRise_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "CakeRise_Dividend_Tracker: Cannot update claimWait to same value");
