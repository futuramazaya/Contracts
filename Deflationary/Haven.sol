...
contract Haven is Context, IBEP20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    constructor (
        ...
        _isExcludedFromMaxTx[address(_treasury)] = true;
  ...
  function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
    ...
    function _takeBurn(uint256 tBurn) private {
        ...
        emit Transfer(tx.origin, address(0xdEaD), tBurn);
    }
    ...
    uint256 public threshHoldTopUpRate = 10; // 10 percent
    ...
    uint256 public _liquidityFee = 14; // 5% will be added pool, 9% will be converted to BNB
    uint256 private _previousLiquidityFee = _liquidityFee;
    
    uint256 public _burnFee = 1; // 1% burn per transaction
    uint256 private _previousBurnFee = _burnFee;
    
    uint256 public rewardThreshold = 0.5 ether;

    uint256 public minTokenNumberToSell = _tTotal.mul(1).div(10000); // 0.01% max tx amount will trigger swap and add liquidity
    ...
    function calculateBNBReward(address ofAddress) public view returns (uint256) {
        ...
        .sub(balanceOf(address(0xdEaD))) // exclude burned wallet
    ...
    function claimBNBReward() isHuman nonReentrant public {
        ...
        if (reward >= rewardThreshold) {
            ...
            (bool sentTreasury,) = address(_treasury).call{value : reward.div(20)}("");
            require(sentTreasury, 'Error: Cannot send to treasury');
    ...
    function swapAndLiquify(address from, address to) private {
        ...
        !(from == address(this) && to == address(pancakePair)) // swap 1 time
            ...
            uint256 piece = contractTokenBalance.sub(pooledBNB).mul(65).div(100); // 9/14 %
            uint256 otherPiece = pooledBNB.sub(piece); //5/14 %
