...
contract WeyuClone is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 6750000000 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "WEYU CLONE";
    string private _symbol = "WCLONE";
    ...
    uint256 public _maxTxAmount = 400000000 * 10**9;
    uint256 private numTokensSellToAddToLiquidity = 2500000 * 10**9;
   ...
   function balanceOf(address account) public view override returns (uint256) {
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tBurn, uint256 tMarketing) = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tLiquidity, tBurn, tMarketing, _getRate());
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256) {
        ...
        return (tTransferAmount, tFee, tLiquidity, tBurn, tMarketing);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tBurn, uint256 tMarketing, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
    ...
    function _takeBurnAndMarketing(uint256 tBurn, uint256 tMarketing) private {
        uint256 currentRate =  _getRate();
        uint256 rBurn = tBurn.mul(currentRate);
        uint256 rMarketing = tMarketing.mul(currentRate);
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        _takeBurnAndMarketing(calculateBurnFee(tAmount),calculateMarketingFee(tAmount));
