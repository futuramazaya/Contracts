...
contract Pyragon is Context, IBEP20, Ownable {
    ...
    uint256 internal DividendPerShare; // Amount of dividend BNB per share. Calculated according to decimals
    ...
    string private _name = "Pyragon Token";
    string private _symbol = "PYRA";
    uint8 private _decimals = 9;
    
    uint256 public _taxFee = 3;
    uint256 private _previousTaxFee = _taxFee;
    
    uint256 public _investorFee = 3;
    uint256 private _previousInvestorFee = _investorFee;
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tInvestment) = _getValues(tAmount);
        _tOwned[sender] = _tOwned[sender].sub(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _tOwned[recipient] = _tOwned[recipient].add(tTransferAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);        
        _takeLiquidity(tLiquidity);
        _takeInvestment(tInvestment);
    ...
    function setInvestmentFeePBEPent(uint256 investorFee) external onlyOwner() {
        _investorFee = investorFee;
    }
   
    function setMaxTxPBEPent(uint256 maxTxPBEPent) external onlyOwner() {
        _maxTxAmount = _tTotal.mul(maxTxPBEPent).div(
            10**3
        );
    }
    
    function setMinAmountToLiquify(uint256 minLiqPBEPent) external onlyOwner() {
        numTokensSellToAddToLiquidity = _tTotal.mul(minLiqPBEPent).div(
            10**5
        );
    }
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tInvestment) = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tLiquidity, tInvestment, _getRate());
        return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tLiquidity, tInvestment);
    }

    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        uint256 tFee = calculateTaxFee(tAmount);
        uint256 tLiquidity = calculateLiquidityFee(tAmount);
        uint256 tInvestment = calculateInvestorFee(tAmount);
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity).sub(tInvestment);
        return (tTransferAmount, tFee, tLiquidity, tInvestment);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tInvestment, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rFee = tFee.mul(currentRate);
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        uint256 rInvestment = tInvestment.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rLiquidity).sub(rInvestment);
    ...
    function _takeInvestment(uint256 tInvestment) private {
        uint256 currentRate =  _getRate();
        uint256 rInvestment = tInvestment.mul(currentRate);
        _rOwned[investmentWallet] = _rOwned[investmentWallet].add(rInvestment);
        if(_isExcluded[address(this)]){
            _tOwned[investmentWallet] = _tOwned[investmentWallet].add(tInvestment);
        }
        emit Transfer(tx.origin, investmentWallet, tInvestment);  
    }
    ...
    function calculateInvestorFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_investorFee).div(
            10**2
        );
    }
    
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _investorFee == 0) return;
    ...
    function _transfer(
        ...
        DividendCorrections[to] = DividendCorrections[to].sub(Correction);
...
//---------- DIVIDEND FUNCTIONS ------------//
    ...
    //Function to add BNB to be distributed as dividends
    //This must be called to distribute dividends. Normal send to contract address will not work!!
    function distributeDividends() public payable {
        if (msg.value > 0) {
            uint256 divs = msg.value;
