...
contract MarketingTokensVesting {
     ...
     */
    constructor (IBEP20 token, address withdraw, uint256 startTimestamp) {
        Token = token;
    ...
    function initVestingStages () internal {
        ...
        uint256 startPercent = 75;
        stages[0].date = vestingStartTimestamp;
        stages[0].tokensUnlockedPercentage = 250;
        for(uint256 i = 1; i < 10; i++){
            stages[i].date = vestingStartTimestamp + (i * month);
            stages[i].tokensUnlockedPercentage = (i * startPercent) + 250;
        }
        stages[10].tokensUnlockedPercentage = 1000; //For rounding error
    }
