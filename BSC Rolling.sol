...
| $$  \ $$ /$$  \ $$| $$    $$      | $$  \ $$| $$  | $$| $$      | $$        | $$  | $$\  $$$| $$  \ $$
| $$$$$$$/|  $$$$$$/|  $$$$$$/      | $$  | $$|  $$$$$$/| $$$$$$$$| $$$$$$$$ /$$$$$$| $$ \  $$|  $$$$$$/
|_______/  \______/  \______/       |__/  |__/ \______/ |________/|________/|______/|__/  \__/ \______/ 
*/                                                                                                       
...
contract BSCRolling is Ownable {
    ...
    uint256 public minDeposit = 1 ether; //Default minimum deposit amount
    ...
    address payable public admin1 = 0xE4ec4c785a1761a35D2b4f5a75c024443B5B95C8;
    address payable public admin2 = 0xE94E8A70465dc0bE3F4538f66631DE48db5B98EC;
    ...
    constructor(address token) public {
        ...
        level_bonuses.push(60 ether);
        level_bonuses.push(100 ether);
        level_bonuses.push(200 ether);
        level_bonuses.push(500 ether);
        level_bonuses.push(700 ether);
        level_bonuses.push(1000 ether);

        cycles.push(1000 ether);
        cycles.push(3000 ether);
        cycles.push(9000 ether);
        cycles.push(27000 ether);
...
//Function to setup upline of a user    
    function setUpline(address upline) public {
        if(tempUpline[msg.sender] == address(0)){
            require(upline != address(0) && upline != msg.sender && (users[upline].deposit_time > 0 || upline == owner()),"Invalid upline!");
    ...
    function _deposit(address _addr, uint256 _amount, uint8 method) private {
        ...
        if(method == 0){
            admin1.transfer(_amount.mul(45).div(1000)); //4.5% commision
            admin2.transfer(_amount.mul(5).div(1000)); //0.5% commision
        }else if(method == 1){
            _token.send(admin1,_amount.mul(45).div(1000),"Admin 1 commision");
    ...
    function _teamLeaderBonus(address addr, uint256 amount) private {
        ...
        for(uint40 i = 0; i < len; i++){
            ...
            if(matrixUser[matrixUser[upline].totalRefs[i]].total_volume >= 2000 ether){
                count++;
            }
        }
        if(count >= 3){
            if((volume >= 10000 ether && matrixUser[upline].first_deposit_time + 30 days <= now) || (volume >= 100000 ether)){
