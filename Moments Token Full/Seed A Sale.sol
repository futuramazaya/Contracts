...
contract MomentsSeedASale is Context, ReentrancyGuard, Owned {
    using SafeMath for uint256;
    ...
    uint256 public StartTimestamp = 1621067620;
    ...
    function withdrawRemainingTokenForOwner() public nonReentrant{
        ...
        isActive = false;
        _token.transfer(owner, remainingBalance);
        totalBalance = 0;
        remainingBalance = 0;
        initStages();
    }
    ...
    function withdrawTokens () public {
        require(!isActive,"Sale is still active");
    ...
    function initStages () internal {
        ...
        stages[7].date = StartTimestamp + (7 * month);
        stages[8].date = StartTimestamp + (8 * month);
        stages[9].date = StartTimestamp + (9 * month);
        

        stages[0].tokensUnlockedPercentage = 100; 
        stages[1].tokensUnlockedPercentage = 200;
        stages[2].tokensUnlockedPercentage = 300;
        stages[3].tokensUnlockedPercentage = 400;
        stages[4].tokensUnlockedPercentage = 500;
        stages[5].tokensUnlockedPercentage = 600;
        stages[6].tokensUnlockedPercentage = 700;
        stages[7].tokensUnlockedPercentage = 800;
        stages[8].tokensUnlockedPercentage = 900;
        stages[9].tokensUnlockedPercentage = 1000;
    ...
    function getTokensUnlocked(address addr) public view returns(uint256 tokensToSend) {
        ...
        if (allowedPercent >= 1000) {
            ...
            uint256 totalTokensAllowedToWithdraw = users[addr].initialBalance.mul(allowedPercent).div(1000);
