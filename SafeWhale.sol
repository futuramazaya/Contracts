...
contract SafeWhale is Context, IERC20, Ownable {
    ...
    string private _symbol = "SWHALE";
    uint8 private _decimals = 9;
    
    uint256 public _taxFee = 3; // 4% redistributed among hodlers
    uint256 private _previousTaxFee = _taxFee;
    
    uint256 public _liquidityFee = 6; // 6% added to liquidity 
    uint256 private _previousLiquidityFee = _liquidityFee;
    
    uint256 public _devFee = 3; // 3% dev fee
    ...
    event devAndmarketing(address _dev, uint256 dev);
...
//Messy code to avoid stack to deep :)
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        uint256 tTransferAmount = tAmount.sub(calculateTaxFee(tAmount)).sub(calculateLiquidityFee(tAmount)).sub(calculatedevFee(tAmount));
        uint256 currentRate = _getRate();
        ...
        rTransferAmount = rTransferAmount.sub(calculatedevFee(tAmount).mul(currentRate));
    ...
    function _transfer(
        ...
        //transfer amount, it will take tax, liquidity, dev fee
        _tokenTransfer(from,to,amount,takeFee);
        if(takeFee){
            _tOwned[_dev] = _tOwned[_dev].add(calculatedevFee(amount));
            _rOwned[_dev] = _rOwned[_dev].add(calculatedevFee(amount).mul(_getRate()));
            emit Transfer(from, _dev, calculatedevFee(amount));
        }
    }
